/**
 * HD Wallet WASM - Official Test Vectors
 *
 * Tests against official BIP-39 test vectors from:
...
// Run BIP-39 test vectors
for (const vector of bip39Vectors) {
  const wordCount = vector.mnemonic.split(' ').length;

  test(`BIP-39 vector: ${wordCount}-word mnemonic (entropy: ${vector.entropy.slice(0, 8)}...)`, () => {
    // Test entropy to mnemonic
    const entropy = hexToBytes(vector.entropy);
    const mnemonic = wallet.mnemonic.fromEntropy(entropy);
    assertEqual(mnemonic, vector.mnemonic, 'Mnemonic from entropy should match');

    // Test mnemonic validation
    assert(wallet.mnemonic.validate(mnemonic), 'Mnemonic should be valid');

    // Test mnemonic to entropy
    const recoveredEntropy = wallet.mnemonic.toEntropy(mnemonic);
    assertEqual(bytesToHex(recoveredEntropy), vector.entropy, 'Entropy from mnemonic should match');

    // Test seed derivation with TREZOR passphrase
    const seed = wallet.mnemonic.toSeed(mnemonic, 'TREZOR');
    assertEqual(bytesToHex(seed), vector.seed, 'Seed should match test vector');
  });
}

// Additional BIP-32 test vectors

// Test Vector 3 (BIP-32)
const testVector3Seed = hexToBytes('4b381541583be4423346c643850da4b320e46a87ae3d2a4e6da11eba819cd4acba45d239319ac14f863b8d5ab5a0d0c64d2e8a1e7d1457df2e5a3c51c73235be');

test('BIP-32 Test Vector 3 - Master', () => {
  const master = wallet.hdkey.fromSeed(testVector3Seed);

  const xpub = master.toXpub();
  assertEqual(xpub, 'xpub661MyMwAqRbcEZVB4dScxMAdx6d4nFc9nvyvH3v4gJL378CSRZiYmhRoP7mBy6gSPSCYk6SzXPTf3ND1cZAceL7SfJ1Z3GC8vBgp2epUt13',
    'Test Vector 3 master xpub should match');

  const xprv = master.toXprv();
  assertEqual(xprv, 'xprv9s21ZrQH143K25QhxbucbDDuQ4naNntJRi4KUfWT7xo4EKsHt2QJDu7KXp1A3u7Bi1j8ph3EGsZ9Xvz9dGuVrtHHs7pXeTzjuxBrCmmhgC6',
    'Test Vector 3 master xprv should match');

  master.wipe();
});

test('BIP-32 Test Vector 3 - m/0\'', () => {
  const master = wallet.hdkey.fromSeed(testVector3Seed);
  const child = master.deriveHardened(0);

  const xpub = child.toXpub();
  assertEqual(xpub, 'xpub68NZiKmJWnxxS6aaHmn81bvJeTESw724CRDs6HbuccFQN9Ku14VQrADWgqbhhTHBaohPX4CjNLf9fq9MYo6oDaPPLPxSb7gwQN3ih19Zm4Y',
    'Test Vector 3 m/0\' xpub should match');
...
test('Standard Bitcoin path: m/44\'/0\'/0\'/0/0', () => {
  ...
  assertEqual(key.depth, 5, 'Depth should be 5');
  assert(!key.isNeutered, 'Key should not be neutered');

  key.wipe();
  master.wipe();
});

test('Standard Ethereum path: m/44\'/60\'/0\'/0/0', () => {
  const seed = hexToBytes('000102030405060708090a0b0c0d0e0f');
  const master = wallet.hdkey.fromSeed(seed);
  const key = master.derivePath("m/44'/60'/0'/0/0");
...
console.log('  (24 BIP-39 vectors + BIP-32 vectors tested)');
