...
function sampleRecordInput() {
  return {
    ...
    registryTag: 'sdn-candidate-2.0.26',
    sourceTag: 'v2.0.26',
    uiPackage: samplePackage('hd-wallet-ui', SHA512_B, SHA_F),
    uiDependencyVersion: '2.0.26',
    version: '2.0.26',
...
test('release CLI modes are strict and mutually exclusive', () => {
  assert.deepEqual(parseArguments([
    '--version', '2.0.26', '--source-ref', 'HEAD', '--skip-tag',
  ]), {
    emitArtifacts: null,
    evidenceRecord: null,
  ...
  assert.throws(
    () => parseArguments(['--version', '2.0.26', '--tag', 'v2.0.26', '--skip-tag']),
    /mutually exclusive/u,
  );
  assert.throws(() => parseArguments(['--version', '2.0.26', '--tag', 'v2.0.27']), /tag/u);
  assert.throws(() => parseArguments(['--version', '2.0.26', '--surprise']), /unknown/u);
  assert.throws(
    () => parseArguments(['--version', '2.0.26', '--workflow-artifacts', '/tmp/a']),
    /expected-run-id/u,
  );
});
...
test('all source and packed versions must be the reviewed pair', () => {
  const contract = {
    ...
    lockCoreVersion: '2.0.26',
    lockRootVersion: '2.0.26',
    lockUiVersion: '2.0.26',
    packedCorePackage: {
      dependencies: { flatbuffers: '25.9.23' },
      name: 'hd-wallet-wasm',
      version: '2.0.26',
    ...
    packedUiPackage: {
      dependencies: { 'hd-wallet-wasm': '2.0.26' },
      name: 'hd-wallet-ui',
      version: '2.0.26',
    },
    relayPackage: { name: '@sdn/wallet-relay', version: '2.0.26' },
    rootPackage: { version: '2.0.26' },
    uiPackage: {
      dependencies: { 'hd-wallet-wasm': '2.0.26' },
      name: 'hd-wallet-ui',
      version: '2.0.26',
  ...
  assert.equal(validateVersionContract(contract, '2.0.26'), true);
  assert.throws(
    () => validateVersionContract({
      ...contract,
      packedUiPackage: { ...contract.packedUiPackage, dependencies: { 'hd-wallet-wasm': '^2.0.26' } },
    }, '2.0.26'),
    /dependency/u,
  );
  assert.throws(
  ...
  assert.throws(
    () => validateVersionContract({ ...contract, cmakeVersion: '2.0.21' }, '2.0.26'),
    /version/u,
  );
});
...
test('release record has only the immutable post-publication fields', () => {
  ...
  for (const [label, mutate] of [
    ...
    ['dependency', (value) => { value.uiDependencyVersion = '^2.0.26'; }],
  ]) {
...
test('workflow artifact layout is an exact three-directory allowlist', () => {
  const entries = [
    'npm-tarballs/hd-wallet-ui-2.0.26.tgz',
    'npm-tarballs/hd-wallet-wasm-2.0.26.tgz',
    'origin-service/sdn-wallet-origin-2.0.26-node24-linux-x64.tar.gz',
    'origin-service/sdn-wallet-origin-2.0.26-node24-linux-x64.tar.gz.sha256',
    ...
    'wallet-release-report/release/static/assets/hd-wallet-ui/2.0.26/client.js',
  ];
  assert.equal(validateWorkflowArtifactEntries(entries, '2.0.26'), true);
  assert.throws(
    () => validateWorkflowArtifactEntries([...entries, 'surprise/file'], '2.0.26'),
    /unexpected artifact/u,
  );
  assert.throws(
    () => validateWorkflowArtifactEntries([...entries, 'npm-tarballs/nested/evil.tgz'], '2.0.26'),
    /nested/u,
  );
  assert.throws(
    () => validateWorkflowArtifactEntries(entries.filter((path) => !path.includes('hd-wallet-ui')), '2.0.26'),
    /missing/u,
  );
});
...
test('workflow artifact report binds run, tag, commit, platform, toolchain, and hashes', () => {
  const report = {
    artifacts: {
      coreTarball: { path: 'npm-tarballs/hd-wallet-wasm-2.0.26.tgz', sha256: SHA_A, sha512: SHA512_A },
      originService: { path: 'origin-service/sdn-wallet-origin-2.0.26-node24-linux-x64.tar.gz', sha256: SHA_B, sha512: SHA512_B },
      uiTarball: { path: 'npm-tarballs/hd-wallet-ui-2.0.26.tgz', sha256: SHA_C, sha512: SHA512_B },
    },
    commit: COMMIT,
