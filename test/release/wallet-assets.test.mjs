...
const staticVersionDirectory = join(
  releaseDirectory,
  'static/assets/hd-wallet-ui/2.0.25',
);
const manifestPath = join(releaseDirectory, 'wallet-assets.v1.json');
const staticUrlPrefix = 'https://static.spacedatanetwork.org/assets/hd-wallet-ui/2.0.25/';
...
test('wallet fragment has the exact JCS schema and three content-addressed local assets', async () => {
  ...
  assert.equal(manifest.walletVersion, '2.0.25');
...
test('wallet asset generation is deterministic and the verifier does not mutate the release', async (t) => {
  ...
  await buildWalletAssets({ repositoryDirectory, releaseDirectory: firstRelease, version: '2.0.25' });
  await buildWalletAssets({ repositoryDirectory, releaseDirectory: secondRelease, version: '2.0.25' });
  const firstFiles = [
  ...
  await verifyWalletAssets({ repositoryDirectory, releaseDirectory: firstRelease, version: '2.0.25' });
...
for (const [name, mutate, pattern] of [
  [
    ...
    async (fixtureRelease) => writeFile(
      join(fixtureRelease, 'static/assets/hd-wallet-ui/2.0.25/extra.js'),
      'extra',
    ),
  ...
  [
    ...
    async (fixtureRelease) => {
      ...
      const bytes = await readFile(localAssetPath(
        manifest.assets.publicClientStyle,
        join(fixtureRelease, 'static/assets/hd-wallet-ui/2.0.25'),
      ));
