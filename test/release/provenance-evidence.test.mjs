...
const EXPECTED_REF = 'refs/tags/v2.0.30';
const EXPECTED_WORKFLOW_IDENTITY =
  'https://github.com/DigitalArsenal/hd-wallet-wasm/.github/workflows/npm-publish.yml@refs/tags/v2.0.30';
...
function buildFulcioLeaf(overrides = {}) {
  ...
  const fields = {
    ...
    tokenSubject: leafExtension(
      OID.tokenSubject,
      derUtf8String('repo:DigitalArsenal/hd-wallet-wasm:ref:refs/tags/v2.0.30'),
    ),
    ...fieldOverrides,
  };
  const extensionRows = [
...
test('synthetic input mirrors the exact npm 11.16 keyed publish bundle shape', () => {
  for (const row of syntheticFixture.audit.verified) {
    ...
    assert.equal(statement.subject[0].name, `pkg:npm/${row.name}@2.0.30`);
...
test('builds deterministic sanitized JCS evidence and revalidates it offline', () => {
  ...
  assert.deepEqual(evidence.workflow, {
    identity: 'https://github.com/DigitalArsenal/hd-wallet-wasm/.github/workflows/npm-publish.yml@refs/tags/v2.0.30',
    name: 'npm-publish.yml',
    path: '.github/workflows/npm-publish.yml',
    ref: 'refs/tags/v2.0.30',
  });
  ...
  for (const entry of evidence.packages) {
    ...
    assert.equal(entry.version, '2.0.30');
...
test('validates either exact package independently for idempotent same-run recovery', () => {
  ...
  for (const packageName of ['hd-wallet-ui', 'hd-wallet-wasm']) {
    const input = validInput();
    input.packageLock.packages[''].dependencies = { [packageName]: '2.0.30' };
...
test('accepts one deduplicated public key for production npm signature and attestation usages', () => {
  ...
  for (const row of input.audit.verified) {
    ...
    registry.dist.signatures = [{
      ...
      sig: signData(
        'sha256',
        Buffer.from(`${row.name}@2.0.30:${registry.dist.integrity}`, 'utf8'),
        publishPrivateKey,
      ).toString('base64'),
    }];
  }
...
test('rejects validly re-signed npm publish statements with wrong exact bindings', async (t) => {
  const cases = [
    ...
    ['subject name', /publish subject name/iu,
      (statement) => { statement.subject[0].name = 'pkg:npm/not-the-wallet@2.0.30'; }],
...
test('rejects malformed, missing, duplicate, and npm-schema-drifted evidence', async (t) => {
  const cases = [
    ...
    ['UI dependency drift', /exact core dependency/iu, (input) => { input.packageLock.packages['node_modules/hd-wallet-ui'].dependencies['hd-wallet-wasm'] = '^2.0.30'; }],
